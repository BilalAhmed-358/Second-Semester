...
public:
    ...
    void booklist(int i)
    {
        ...
        if (!intf)
            cout << "\n\t\tThere was an error in opening the file, FILE NOT FOUND.";
        else
        {
            cout << "\n\t    ************ Book List ************ \n\n";
    ...
    void modify()
    {
        ...
        system("cls");
        cout << "\n\t\t>>Please Choose one option :-\n";
        cout << "\n\t\t1.Modification In Existing Books\n\n\t\t2.Add New Book in the booklist\n\n\t\t3.Delete A Book from the booklist\n\n\t\t4.Go back to Librarian Menu\n";
        cout << "\n\n\t\tChoose an option from the menu below: ";
        cin >> i;
        if( i== 1)
        {
            ...
            if (cont == 0)
            {
                cout << "\n\t\tA book with the name you entered does not exist in the database\n";
                cout << "\n\t\tPress any key to continue.....";
                getch();
                system("cls");
                modify();
        ...
        else if( i== 2)
        {
            ...
            outf.close();
            cout << "\n\t\tBook added Successfully in the database!\n";
        }
        else if( i== 3)
        {
            system("cls");
            ...
            else if (i == 2)
            {
                ...
                intf.close();
                outf.close();
                remove("Booksdata.txt");cout<<"This ran\n";
                rename("temp.txt", "Booksdata.txt");cout<<"This ran\n";
            }
            else
            {
                cout << "\n\t\tIncorrect Input.....:(\n";
    ...
    void book_issue()
    {
        ...
        system("cls");
        cout << "\n\t\t->Please Choose one option :-\n";
        cout << "\n\t\t1.Issue a Book to student\n\n\t\t2.View Issued Books to student(s)\n\n\t\t3.See students who issued books\n\n\t\t4.Reissue Book\n\n\t\t5.Return Book\n\n\t\t6.Go back to Librarian menu\n\n\t\tEnter Your Choice : ";
        cin >> i;
        fflush(stdin);
        if (i == 1)
        {
            ...
            cin.getline(book_id, 20);
            //strcpy(st,book_id);
            Manage_quantity_of_books(book_id, type_of_book_chosen_by_user, 1);
            cout << "\n\t\tEnter Student Name : ";
            ...
            outf.write((char *)this, sizeof(*this));
            outf.close();
            cout << "\n\n\t\tBook Issued Successfully to "<<authorname<<".\n";
        }
        else if (i == 2)
        {
        ...
        else if (i == 4)
        {
            ...
            while (!intf.eof())
            {
                ...
                if (book_id[i] == '\0' && student_id[j] == '\0' && st[j] == '\0' && st1[i] == '\0')
                {
                    ...
                    cout << "\n\t\tEnter New Date : ";
                    cin >> quantity_of_book >> Type_of_book_id >> price_of_books;
                    Fine_Calculation(d, m, y, quantity_of_book, Type_of_book_id, price_of_books); //fn1
        ...
        else if (i == 5)
        {
            ...
            cout << "\n\t\tEnter Book's ID : ";
            cin.getline(st1, 20);
            Manage_quantity_of_books(st1, type_of_book_chosen_by_user, 2);
            cout << "\n\n\t\tEnter Student's ID : ";
            cin.getline(st, 20);
            cout << "\n\t\tEnter Present date : ";
            cin >> d >> m >> y;
            ...
            while (!intf.eof())
            {
                ...
                if (book_id[i] == '\0' && student_id[j] == '\0' && st[j] == '\0' && st1[i] == '\0' && cont == 0)
                {
                    cont++;
                    intf.read((char *)this, sizeof(*this));
                    Fine_Calculation(quantity_of_book, Type_of_book_id, price_of_books, d, m, y);
    ...
    void Manage_quantity_of_books(char st[], int type_of_book_chosen_by_user, int x)
    {
        int i, cont = 0;
        fstream intf("Booksdata.txt", ios::in | ios::out | ios::ate | ios::binary);
        intf.seekg(0);
    ...
    void Fine_Calculation(int d, int m, int y, int dd, int mm, int yy)
    {
        long int n1, n2;
        int years, l, i;
        const int monthDays[12] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};//Days of the month in whole year
        n1 = y * 365 + d;
        for (i = 0; i < m - 1; i++)
